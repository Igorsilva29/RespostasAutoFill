// content.js — Inserção robusta multi-campo + WhatsApp fix
...
async function insertTextOnce(element, text) {
  ...
  try {
    ...
    if (element.isContentEditable) {
      ...
      // Detecta se é WhatsApp Web (campo específico)
      const isWhatsApp = element.closest('[contenteditable="true"][data-tab]') || false;
      if (isWhatsApp) {
        // Usando Clipboard API para colar multiline
      ...
      // Outros contenteditables (Gestão / Email)
      ...
      if (sel && sel.rangeCount > 0) {
        ...
        lines.forEach((line, idx) => {
          ...
          if (idx < lines.length - 1) range.insertNode(document.createElement("br"));
        });

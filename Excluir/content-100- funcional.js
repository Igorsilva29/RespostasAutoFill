// content.js — Injeção e inserção robusta (multi-campo + WhatsApp fix + inversão corrigida)
// Atalho Ctrl + /
...
// ---------------------- Injeção do script no documento
...
async function insertTextOnce(element, text) {
  ...
  try {
    ...
    if (element.isContentEditable) {
      ...
      // Email/Gestão — mantém ordem correta
      const sel = element.ownerDocument.getSelection();
      if (sel && sel.rangeCount > 0) {
        ...
        // IMPORTANTE: adiciona linhas na ordem correta (não inverter)
        text.split("\n").forEach((line, idx, arr) => {
          fragment.appendChild(document.createTextNode(line));
          if (idx < arr.length - 1) fragment.appendChild(element.ownerDocument.createElement("br"));
        });

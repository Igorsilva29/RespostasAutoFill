// content.js — injeção e inserção robusta (multi-campo)
// Atalho Ctrl + /
...
// ---------------------- Remover trigger "/" antes da inserção
...
// ---------------------- Inserção específica por tipo de campo
async function insertTextOnce(element, text) {
  ...
  try {
    ...
    // 2) ContentEditable moderno (WhatsApp, Gmail, Email)
    if (element.isContentEditable) {
      element.focus();
      // Tenta insertText via execCommand (compatível com WhatsApp)
